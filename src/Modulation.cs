...
{
    [ComVisible(false)]
    public class Modulation : Header
    {
        internal IntPtr ModPtr => handle;

        internal Modulation(IntPtr modulation) : base(modulation)
        {
        }
        ...
        public static Modulation SineSquared(int freq, double amp = 1, double offset = 0.5f)
        {
            NativeMethods.AUTDModulationSineSquared(out var modPtr, freq, amp, offset);
            return new Modulation(modPtr);
        }
